...
class DataFeedDownloaderTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testTheDataFileCanBeRetrievedAfterDownloading()
    {
        ...
        $data_feed_downloader = new DataFeedDownloader($this->curl, 'http://www.example.com', '/foo.txt');

        $this->assertEquals('foo', $data_feed_downloader->data());
    }

    public function testAnExceptionWithTheHttpStatusCodeIsThrownIfTheDataFileCannotBeDownloaded()
    {
        $this->curl->shouldReceive('getInfo')->andReturn(404);
        $data_feed_downloader = new DataFeedDownloader($this->curl, 'http://www.french-property.com/invalidfilename.txt', '/foo.txt');
    ...
    public function estTheLastDownloadedFileCanBeRetrievedFromTheFileSystemIfTheLiveDataFileWasNotDownloaded()
    {
        $this->curl->shouldReceive(['execute' => 'foo', 'getInfo' => 200]);
